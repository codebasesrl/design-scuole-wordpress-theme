...
function dsi_register_circolare_archimede_post_type()
{

    /** circolari **/
    $labels = array(
        'name' => _x('CircolariArchimede', 'Post Type General Name', 'design_scuole_italia'),
        'singular_name' => _x('CircolareArchimede', 'Post Type Singular Name', 'design_scuole_italia'),
        'add_new' => _x('Aggiungi una CircolareArchimede', 'Post Type Singular Name', 'design_scuole_italia'),
        'add_new_item' => _x('Aggiungi una nuova Circolare Archimede', 'Post Type Singular Name', 'design_scuole_italia'),
        'edit_item' => _x('Modifica la Circolare Archimede', 'Post Type Singular Name', 'design_scuole_italia'),
    );
    $args = array(
        'label' => __('CircolareArchimede', 'design_scuole_italia'),
        'labels' => $labels,
        ...
        'has_archive' => true,
        'capability_type' => array('circolareArchimede', 'circolariArchimede'),
        'map_meta_cap' => true,
    );
    register_post_type('circolareArchimede', $args);
 ...
 */
add_action( 'cmb2_init', 'dsi_add_circolarArchimedee_metaboxes' );
function dsi_add_circolare_archimede_metaboxes() {

    $prefix = '_dsi_circolare_archimede';
...
add_action( 'save_post', 'dsi_feedback_circolare_archimede', 100, 2 );

function dsi_feedback_circolare_archimede( $post_id, $post )
{
    ...
    if ( 'circolareArchimede' !== $post->post_type )
 ...
 * Firma circolare
 */
add_action("get_header", "dsi_save_sign_init_archimede", 100);

function dsi_save_sign_init_archimede(){
...
function add_custom_meta_box_archimede()
{
    add_meta_box("circolari-meta-box", "Report Circolari", "dsi_circolari_archimede_meta_box", "circolare", "normal");
}

add_action("add_meta_boxes", "add_custom_meta_box");
function dsi_circolari_archimede_meta_box($post)
{
 ...
 */
function dsi_circolari_archimede_admin_notice(){
    global $pagenow;
    if ( $pagenow == 'post.php' ) {
        global $post;
 ...
 */
add_action( 'edit_form_after_title', 'sdi_circolare_archimede_add_content_after_title' );
function sdi_circolare_archimede_add_content_after_title($post) {
    if($post->post_type == "circolare")
 ...
 */
add_action( 'edit_form_after_title', 'sdi_circolare_archimede_add_content_before_editor', 100 );
function sdi_circolare_archimede_add_content_before_editor($post) {
    if($post->post_type == "circolare")
 ...
 */
add_action( 'edit_form_after_editor', 'sdi_circolare_archimede_add_content_after_editor', 100 );
function sdi_circolare_archimede_add_content_after_editor($post) {
    if($post->post_type == "circolare")
 ...
 * aggiungo js
 */
add_action( 'admin_print_scripts-post-new.php', 'dsi_circolare_archimede_admin_script', 9 );
add_action( 'admin_print_scripts-post.php', 'dsi_circolare_archimede_admin_script', 9 );

function dsi_circolare_archimede_admin_script() {
    global $post_type;
...
add_filter( 'post_row_actions', 'dsi_circolare_archimede_modify_list_row_actions', 10, 2 );
function dsi_circolare_archimede_modify_list_row_actions( $actions, $post ) {
    if ( $post->post_type == "circolare" ) {
        $notificato =  get_post_meta($post->ID, "_dsi_notificato", "true");
